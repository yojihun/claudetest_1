...
def content_blocks() -> list[Block]:
    return [
        ...
        Block("bullet", "Chapter I. 병원통계 기초와 용어 사전: 기초 지식, 병상, 재원일수, 사망, 외래, 산과 지표의 쉬운 뜻과 공식을 총망라한다."),
        Block("bullet", "Chapter II. 계산 방법 실전 훈련: 비율형, 평균형, 회전형으로 공식을 분류하고 실전 문제 해결을 위한 단계별 루틴을 익힌다."),
        Block("bullet", "Chapter III. 빈출 문제 자가 점검: 국가시험에서 자주 출제되는 핵심 질문과 상세 답변을 통해 실전 능력을 최종 확인한다."),
        ...
        Block("term", "척도의 종류: 명명척도(분류), 서열척도(순서), 등간척도(간격 일정, 절대0 없음), 비율척도(절대0 존재, 계산 가능)"),
        ...
        Block("term", "병상이용률 (Bed Occupancy Rate): 준비된 병상이 일정 기간 동안 실제로 얼마나 활용되었는지를 나타내는 지표다.\n공식: (기간 중 총 환자일수 ÷ (가동병상수 × 기간일수)) × 100 (%)"),
        Block("term", "병상회전율 (Bed Turnover Rate): 일정 기간 동안 가동병상 1개당 환자가 몇 번이나 거쳐갔는가(퇴원했는가)를 나타낸다.\n공식: 기간 중 총 퇴원환자수 (사망 포함) ÷ 가동병상수 (회)"),
        Block("term", "병상회전간격 (Bed Turnover Interval): 한 환자가 퇴원하고 다음 환자가 들어올 때까지 병상이 비어 있는 평균 날짜다.\n공식: ((가동병상수 × 기간일수) - 총 환자일수) ÷ 퇴원환자수 (일)"),
        Block("h2", "4. 재원일수 지표의 쉬운 뜻과 공식"),
        Block("term", "일일평균재원환자수 (Average Daily Census): 특정 기간 동안 하루 평균 입원해 있었던 환자의 머릿수다.\n공식: 기간 중 총 환자일수 ÷ 기간일수 (명)"),
        Block("term", "평균재원일수 (Average Length of Stay): 퇴원한 환자 1명이 입원해서 퇴원할 때까지 평균 며칠이 걸렸는지를 뜻한다.\n공식: 퇴원환자의 총 재원일수 ÷ 퇴원환자수 (일)"),
        Block("term", "환자일수 (Inpatient Service Days): 병원에 하루 동안 입원해 있던 환자들의 일수를 모두 누적한 합계다. (예: 3명이 4일씩 입원했다면 환자일수는 12일)"),
        Block("pagebreak", ""),

        Block("kicker", "CHAPTER I"),
        Block("h1", "사망·외래·산과 관련 지표"),
        Block("h2", "5. 사망 관련 지표 (분모·분자 구분의 핵심)"),
        Block("term", "조사망률 (Gross Mortality Rate): 병원 전체 퇴원환자 중 사망자의 비율이다.\n공식: (기간 중 총 사망수 ÷ 총 퇴원환자수(사망 포함)) × 100 (%)"),
        Block("term", "순사망률 (Net Mortality Rate): 불가항력적인 입원 48시간 미만 사망을 제외한 사망률이다.\n공식: (48시간 이상 사망수 ÷ (총 퇴원환자수 - 48시간 미만 사망수)) × 100 (%)"),
        Block("term", "신생아사망률 (Newborn Mortality Rate): 신생아실 퇴원아 중 사망아의 비율이다.\n공식: (신생아실 총 사망수 ÷ 신생아실 총 퇴원수) × 100 (%)"),
        Block("term", "태아사망률 (Fetal Death Rate): 임신 20주 이상 태아가 분만 전 사망한 비율(사산율)이다.\n공식: (태아사망수 ÷ (총 분만수 + 태아사망수)) × 100 (%)"),
        Block("term", "수술사망률 (Postoperative Mortality Rate): 수술 후 10일 이내에 발생한 사망률이다.\n공식: (수술 후 10일 이내 사망수 ÷ 총 수술환자수) × 100 (%)"),
        Block("term", "모성사망률 (Maternal Mortality Rate): 임신·분만·산욕기 합병증으로 사망한 비율이다.\n공식: (모성 사망수 ÷ 모성 퇴원환자수) × 100 (%)"),
        Block("term", "사망지수 (사망퇴원율, Death Index): 전체 퇴원 결과 중 사망퇴원이 차지하는 비율이다.\n공식: (사망퇴원자 수 ÷ 총 퇴원자 수) × 100 (%)"),
        Block("h2", "6. 외래 및 산과 관련 지표"),
        Block("term", "일일평균외래환자수: 특정 기간 동안 하루 평균 외래 진료를 받은 환자 수다.\n공식: 기간 중 총 외래환자수 (연인원) ÷ 기간일수 (명)"),
        Block("term", "제왕절개율 (Cesarean Section Rate): 전체 분만 건수 중 수술로 분만한 비율이다.\n공식: (제왕절개분만수 ÷ 총 분만수) × 100 (%)"),
        Block("pagebreak", ""),
        ...
        Block("h2", "4. 병원통계 단골 오답 유형과 방지법"),
        Block("bullet", "환자일수와 총 재원일수 혼동: 환자일수는 하루 단위 누적이고, 총 재원일수는 퇴원환자 기준의 입원 기간 합이다."),
        Block("bullet", "일수 누락: 이용률이나 회전간격 계산 시 분모/분자에서 기간의 '일수'를 곱하거나 빼먹는 실수를 조심한다."),
        Block("bullet", "48시간 기준 적용: 사망률 문제에서 '조(Gross)'인지 '순(Net)'인지에 따라 48시간 기준 제외 여부를 칼같이 구분한다."),
        Block("bullet", "신생아실 분리: 신생아 통계는 반드시 신생아실에 입원한 신생아만을 대상으로 계산하며 일반 입원아동과 섞지 않는다."),
        Block("pagebreak", ""),

        Block("kicker", "CHAPTER III"),
        Block("h1", "자가 점검 (Q&A 10선)"),
        ...
        Block("answer", "일일평균재원환자수가 160명이므로 30일간 총 환자일수는 160 × 30 = 4,800일이다. 병상이용률 = (4,800 ÷ (200 × 30)) × 100 = 80% 이다. (또는 단순히 일일평균 160명을 가동병상 200개로 나누어 160 ÷ 200 × 100 = 80%로 구해도 무방하다.)"),
